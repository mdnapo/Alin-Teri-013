        ...
        @foreach($items as $item)
            <tr>
                ...
                <td>
                    @if($item->protected == 0)
                        @if($item->active == 1)
                            <a href="{{ url('/admin/pages/visibility/'.$item->id.'/0') }}"
                               class="glyphicon glyphicon-eye-close"></a>
                        @else
                            <a href="{{ url('/admin/pages/visibility/'.$item->id.'/1') }}"
                ...
                <td>
                    @if($item->sort > App\Page::where('archived', 0)->orderBy('sort')->first()->sort)
                        <a href="{{ url('/admin/pages/move-up/'.$item->id) }}"
                           class="glyphicon glyphicon-arrow-up plain_link"></a>
                    @endif
                    @if($item->sort < App\Page::where('archived', 0)->orderBy('sort', 'DESC')->first()->sort)
                        <a href="{{ url('/admin/pages/move-down/'.$item->id) }}"
                           class="glyphicon glyphicon-arrow-down plain_link"></a>
                    @endif
                </td>
                <td>
                    @if($item->protected == 0)
                        <a href="{{ url('/admin/pages/delete/'.$item->id) }}"
                           class="glyphicon glyphicon-remove plain_link"></a>
                    @endif
                </td>
            </tr>
        @endforeach
        ...
        @foreach($archived as $archive)
            ...
            <td>
                <a class="plain_link" data-toggle="tooltip" title="Herstellen"
                   href="{{ url('/admin/pages/restore/' . $archive->id) }}"><span
                            class="material-icons">restore_page</span></a>
                <a class="plain_link" data-toggle="tooltip" title="Pagina bekijken"
                   href="{{ url('/admin/pages/viewArchive/' . $archive->id) }}"><span class="material-icons">open_in_browser</span></a>
            </td>
        @endforeach
        </tbody>
    </table>
